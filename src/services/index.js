...
export const signupWithEmail = async (payload) => {
  const { data } = await http.post(`/auth/signup-email`, payload);
  return data;
};

export const signupWithPhone = async (payload) => {
  const { data } = await http.post(`/auth/signup-phone`, payload);
  return data;
};
...
export const forgetPasswordOTP = async (payload) => {
  const { data } = await http.post('/auth/forgot-verify-otp', payload);
  return data;
};
...
export const loginWithEmail = async (payload) => {
  const { data } = await http.post(`/auth/login-email`, payload);
  return data;
};

export const loginWithPhone = async (payload) => {
  const { data } = await http.post(`/auth/login-phone`, payload);
  return data;
};

export const googleFacebookAuth = async (payload) => {
  const { data } = await http.post(`/auth/auth-login`, payload);
  return data;
};
...
export const getAllUsers = async (page, search) => {
  const { data: response } = await http.get(`/all/users?search=${search}&page=${page}`);
  return response;
};
...
// export const getUserProductDetail = async (pid) => {
//   const { data } = await http.get(`/user/products/${pid}`);
//   return data;
// };
...
export const getProductsByCategoryID = async (query = '', category) => {
  const { data } = await http.get(`/category/products/${category}${query}`);
  return data;
};
export const getProductsByCategory = async (query = '', category, subcategory) => {
  if (!query) query = '?limit=12';
  else query = query + '&limit=12';
  const { data } = await http.get(`/products/category/${category}/subcategory/${subcategory}${query}`);
  return data;
};
...
export const getSimilarProducts = async (productId, subcategory) => {
  const { data } = await http.get(`/products-alike/subcategory/${subcategory}/product/${productId}`);
  return data;
};
...
export const getProductBySlug = async (slug) => {
  // if (!token) {
  const { data } = await http.get(`/user/products/slug/${slug}`);
  return data;
  // } else {
  //   const { data } = await http.get(`/user/products/slug/${slug}`, {
  //     headers: { Authorization: `Bearer ${token}` }
...
export const deleteAddress = async ({ _id }) => {
  const { data } = await http.delete(`/users/address/${_id}`);
  return data;
};
...
export const placeOrderUpdatePayment = async (payload) => {
  const { data } = await http.post(`/orders/update-payment`, payload);
  return data;
};
...
export const getWishlist = async () => {
  const { data } = await http.get(`/users/wishlist`);
  return data;
};
export const updateWishlist = async (payload) => {
  const { data } = await http.post(`/users/wishlist`, payload); // { pid }
  return data;
};
...
export const updateCart = async (payload) => {
  const { data } = await http.put(`/cart/update`, payload);
  return data;
};

export const removeCart = async (payload) => {
  const { data } = await http.delete(`/cart/remove`, {
    data: payload,
    headers: {
...
export const getAllCategories = async () => {
  const { data } = await http.get(`/home/categories`); // /all-categories
  return data;
};
...
export const getBrands = async (query = '') => {
  if (!query) query = 'limit=9';
  else query = query + '&limit=9';
  const { data } = await http.get(`/brands?${query}`);
...
export const paymentSession = async (amount, currency, type, isLocal, addressId) => {
  const { data } = await http.post(`/payment-session`, {
    amount,
    currency,
    type,
    isLocal,
...
export const getUserCategoryByID = async (category) => {
...
export const getCategoryBySlug = async (category) => {
  const { data } = await http.get(`/categories/slug/${category}`);
  return data;
};
...
export const getBrandBySlug = async (slug) => {
  const { data } = await http.get(`/brands/slug/${slug}`);
  return data;
};
...
export const getSubCategoryBySlug = async (subcategory) => {
  const { data } = await http.get(`/subcategories/slug/${subcategory}`);
  return data;
};
...
export const getAllBanners = async () => {
  const { data } = await http.get(`/banner/all`);
  return data;
};

export const addNewAddress = async (payload) => {
  const { data } = await http.post(`/users/address`, payload);
  return data;
};

export const getAllAddresses = async () => {
  const { data } = await http.get(`/users/address`);
  return data;
};

export const updateAddress = async ({ _id, ...payload }) => {
  const { data } = await http.put(`/users/address/${_id}`, payload);
  return data;
};

export const deleteVendor = async (payload) => {
  const { data } = await http.delete(`/users/delete`, { data: payload });
  return data;
};

export const userSupport = async (payload) => {
  const { data } = await http.post(`/users/support`, payload);
  return data;
};

export const searchProducts = async (query = '') => {
  if (!query) query = '?limit=16';
  else query = query + '&limit=16';
  const { data } = await http.get(`/products/search/${query}`);
  return data;
};

export const getShopProducts = async (query = '', sid) => {
  if (!query) query = '?page=1&limit=16';
  else query = query + '&limit=16';
  const { data } = await http.get(`/products/brand/${sid}${query}`);
  return data;
};

// shipping detail
export const getShippingDetails = async (payload) => {
  const { data } = await http.post(`/user/products/pricing`, payload);
  return data;
};

export const getOrderDetail = async (token, oid) => {
  if (!token) {
    const { data } = await http.get(`/orders/${oid}`);
    return data;
  } else {
    const { data } = await http.get(`/orders/${oid}`, {
      headers: { Authorization: `Bearer ${token}` }
...
export const getAllOrders = async (query = '') => {
  if (!query) query = '?limit=12&page=1';
  else query = '?' + query + '&limit=12';

  const { data } = await http.get(`/sub-orders${query}`);
  return data;
};

export const getAllRecentOrders = async (query = '') => {
  if (!query) query = '?limit=10&page=1';
...
export const getSubOrder = async (token, oid) => {

  if (!token) {
    const { data } = await http.get(`/sub-orders/${oid}`);
    return data;
  } else {
    const { data } = await http.get(`/sub-orders/${oid}`, {
      headers: { Authorization: `Bearer ${token}` }
...
export const cancelOrder = async (payload) => {
  const { data } = await http.post(`/orders/cancel`, payload);
  return data;
};

export const buyNowCart = async (payload) => {
  const { data } = await http.post(`/cart/buy-now`, payload);
  return data;
};

export const getCheckoutCart = async (addressId) => {
...
// api used in cart screen when user press proceed to checkout btn to move all items to checkout process
export const updateCartCheckout = async () => {
  const { data } = await http.put(`/cart/update-checkout`);
  return data;
};
...
export const getAllNotifications = async (query = '') => {
  if (!query) query = 'page=1&limit=20';
  else query = query + '&limit=20';
  const { data } = await http.get(`/notifications/user?${query}`);
  return data;
};

export const readAllNotifications = async () => {
  const { data } = await http.post(`/notifications/user/mark-all-opened`);
  return data;
};
